...
class TakeTblParent extends React.Component {
  ...
  componentDidMount(){
     ...
     let actionlist = {"cancel":"handleCancel","ok":"handleOk","savetable":"handleSaveTable","savesection":"handleSaveSection"};
     let fnClick= function(event){ //event for saving the information
       event.preventDefault();
       if (event.target.type==='button' && event.target.getAttribute("action")){
     ...
     document.addEventListener("contextmenu", function(event){
       ...
       let obj = event.target.getAttribute("selected")? event.target : (event.target.parentElement &&  event.target.parentElement.getAttribute('selected'))?event.target.parentElement :null ;
       if(obj){
         event.preventDefault();
         document.addEventListener('click', fnClick);
