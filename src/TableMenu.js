...
class TableMenu extends TakeTblParent {
   ...
   handleKeyDown = (event) => {
    ...
    if (event.target && event.target.value && event.target.maxLength && event.target.value.length>event.target.maxLength-1){
      if (event.keyCode!==8){
        event.preventDefault();
        return false;
      }
...
render(){
  ...
  return(
   ...
   <div className="rightmenutbl" ref={this.contextRef} onClick={this.handleClick} view="group">
    <div className="dropdown">
        ...
        <div view="popup" className="form-group form-group-sm">
            <div view="popup" className="input-group input-group-sm mb-3">
             <div view="popup" className="input-group-prepend">
               <span view="popup" className="input-group-text" id="basic-addon1"><i className="far fa-question-circle itable smallfont"><label className="required">*</label></i></span>
           </div>
             <input  view="popup" name="tableno" type="number"  required={true} className="form-control" size="6"  onKeyDown={ this.handleKeyDown} placeholder={headers[lang].tableno}   maxLength="3" min="1" max="99" aria-describedby="basic-addon1"/>
           </div>
       </div>
       <div view="popup" className="form-group form-group-sm">
           <div view="popup" className="input-group input-group-sm mb-3">
            ...
            <input  view="popup" name="seats" required={true} type="number" className="form-control" size="6"   placeholder={headers[lang].seats} onKeyDown={this.handleKeyDown} maxLength="2" min="1" max="99" aria-describedby="basic-addon1" />
          </div>
      </div>
      <div view="popup" className="form-group form-group-sm">
          <div view="popup" className="input-group input-group-sm mb-3">
     ...
     <div className="btn-group mr-3" role="group" aria-label="tablesave">
        <button className="btn btn-cancel mr-1 btn-sm" type="button"  onClick={this.handleClick}>  {headers[lang].Cancel}</button><button type="button" view="popup" className="btn btn-primary btn-sm" style={styles.right}  action="savetable"  onClick={this.handleSave} >{headers[lang].Ok}</button>
     </div>
     </form>
     <div className="d-none dropdown-menu">
          <div className="dropdown-divider"></div>
          <a className="dropdown-item" href="#">Merge</a>
          <div className="dropdown-divider"></div>
   ...
   /*
   <div className="rightmenutbl">
     <button className="btn btn-cancel mr-1 btn-sm" type="button" action="cancel"  >ClickMe</button>
   </div>*/
 );
 }
}
