...
function getcontent(){
  ...
  let rightmenu = previewNode.querySelector('.rightmenutbl')||previewNode.querySelector('.rightmenusection');
  if (toast){
...
function savefloorplan(msg){
  ...
  floorplan.description=document.querySelector("textarea[name='fpdescription']").value;
  floorplan.default=document.querySelector("input[name='fpdefault']").checked;
  floorplan.published=document.querySelector("input[name='fppublished']").checked;
...
function savetable(e,msg){
  ...
  tblinputs.forEach((input)=>{
    if (input.required==true && input.value==0){
      error=error+'<p style="color:red;">'+  input.placeholder + " " + msg.Empty + "</p>"
    }
    // no duplicates of table number
    tables.forEach((table)=>{
      if (input.name=='tableno' && table.tableno==input.value && tblid!==table.id && table.state=='A'){
        error=error+'<p style="color:red;">'+ input.placeholder + ":" + input.value +" "+msg.AlreadyExist + "</p>";
      }
    });
    inputs.push(input);
  ...
  if (ctable && selectedItem){
    if (selectedItem.getAttribute('class').indexOf("locked")>-1){
      document.getElementById('tbltype').innerHTML='<i class="fa fa-lock itable locked smallfont" aria-hidden="true"></i>';
    }else{
      document.getElementById('tbltype').innerHTML='<i class="fa fa-unlock itable smallfont" aria-hidden="true"></i>';
    }
    ...
    document.getElementById('rseats').innerHTML=parseInt(document.getElementById('rseats').innerHTML)-ctable.seats;
...
var section={
    create:function(id){
      if (!stopdrop){
        ...
        this.id=id;
        sections.push({id:id,name:'',bearer:'',state:'A',date:new Date(),style:div.style});
        return div
      }
      stopdrop = false;
    },
 ...
 function sectiondrop(ev){
   ...
   if (!type){ /// trying to move a section or anyother type into section
     $('.toast').toast({animation:true,autohide:true,delay:1500});
    ...
    if (obj.getAttribute('placed') && obj.parentElement && obj.parentElement.id && obj.parentElement.id.indexOf('content')>-1){
...
function savesection(event,msg){
  ...
  sectioninputs.forEach((input)=>{
    ...
    sections.forEach((sec)=>{
      if (input.value.trim().length>0 && sec.name==input.value && sec.id!=selectedItem.id.replace('section','')){
        error= error+"<p style='color:red;'>" + input.placeholder + " :" + input.value + " "+ msg.AlreadyExists + "</p>" ;
      }
    })
    inputs.push(input);
  })
...
function setweekday(){
  ...
  let weekdays = document.querySelectorAll('.weekdays > tr > td > label > input');
  let checked = false;
  if(weekdays){
...
function setSectionColor(color){
  ...
  if (selectedItem && selectedItem.getAttribute("class").indexOf("section")>-1){
...
function select(ev){
  ...
  if (ev.shiftKey){// checking for multi select
    let mergingItem=null;
    if (selectedItem && selectedItem.getAttribute('type')=='tbl' && ev.target && (ev.target.getAttribute('placed') || ev.target.parentElement.getAttribute('placed') || ev.target.parentElement.parentElement.getAttribute('placed'))){
      if (ev.target.getAttribute('placed')){
        mergingItem=ev.target;
          ...
          if (ev.target.parentElement.getAttribute('placed')){
            ...
            if(ev.target.parentElement.parentElement.getAttribute('placed')){ // rectange and oval
  ...
  if (ev && ev.target&& ev.target.getAttribute && (ev.target.getAttribute('placed')||ev.target.parentElement.getAttribute('placed')||ev.target.parentElement.parentElement.getAttribute('placed'))){
    if (ev.target.getAttribute('placed')){
  ...
  if(selectedItem){
    ...
    if (type=='table'){
      ...
      if (table){
        if (selectedItem.getAttribute('class').indexOf("locked")>-1){
          document.getElementById('tbltype').innerHTML='<i class="fa fa-lock itable locked" aria-hidden="true"></i>';
        }else{
          document.getElementById('tbltype').innerHTML='<i class="fa fa-unlock itable" aria-hidden="true"></i>';
        }
...
function drop(ev) {
      ...
      if  (obj && !obj.getAttribute('placed')){
        ...
        if (obj.getAttribute('class') && obj.getAttribute('class').indexOf('iconsection')>-1){
          ...
          nodeCopy.id = getToolType(obj).length>0?"newId"+getToolType(obj).length:"newId0";
          tables.push({id:nodeCopy.id.replace('newId',""),uid:nodeCopy.id,tableno:'',seats:0,state:'A',date:new Date(),class:nodeCopy.getAttribute('class'),style:nodeCopy.style});
        }
        if (!nodeCopy){
      ...
      obj.style.top=pageY-offsetTop-obj.offsetHeight / 2 + 'px';+'px';
      obj.style.left=pageX-offsetLeft-obj.offsetWidth / 2 + 'px';+'px';
      select(obj);
      ev.preventDefault();
        ...
        if(!document.getElementById("m"+selectedItem.id)){
          ...
          if (!selectedItem.style.top && !selectedItem.style.left){
            if (selectedItem.parentElement.id=="content"){
              ...
              left= parseInt(selectedItem.parentElement.style.left.replace('px',''))+35;
            }else{
                top = selectedItem.parentElement.style.top.replace('px','')
                left= parseInt(selectedItem.parentElement.style.left.replace('px',''))+35;
                let store = selectedItem.parentElement.parentElement;
                if (store && store.id.indexOf('section')>-1){
                 top = parseInt(selectedItem.parentElement.style.top.replace('px',''))+ parseInt(store.style.top.replace('px',''));
                 left= parseInt(selectedItem.parentElement.style.left.replace('px',''))+ parseInt(store.style.left.replace('px','')) + 35;
                }
            }
          }else{
          ...
          if(selectedItem.getAttribute('class').indexOf('itable')>-1){
            ...
            let parentItem = selectedItem.parentElement;
            if (parentItem && parentItem.getAttribute('class').indexOf('section')>-1){
     ...
     if (selectedItem.getAttribute("class").indexOf('section')>-1){
       ...
       tables.forEach((t)=>{
         if (t.id==id){
          ...
          document.getElementById('rseats').innerHTML=parseInt(document.getElementById('rseats').innerHTML)+parseInt(t.seats)
         }
       });
     }
     selectedItem.remove();
