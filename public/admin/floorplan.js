...
function savefloorplan(msg) {
    ...
    floorplan.default = document.querySelector(
        "input[name='fpdefault']"
    ).checked;
  ...
  */
    ...
    fetch(
        "https://dummyapi-sidabs.herokuapp.com/getrestaurant.php/savefloorplan.php",
        {
            method: "POST",
...
function getRestaurants(id) {
    let results = [];
    fetch(
        `https://dummyapi-sidabs.herokuapp.com/getrestaurants.php/?id=${id}`,
        {
            method: "GET",
            dataType: "json"
        }
...
function sectiondrop(ev) {
    ...
    if (!type) {
        ...
        $(".text-muted").text("");
        $(".toast-body").text(
            "Section cannot be placed inside another section"
        );
        $(".toast").toast("show");
        stopdrop = true;
        ev.preventDefault();
...
function select(ev) {
        ...
        if (
            ...
            if (ev.target.getAttribute("placed")) {
                ...
                if (ev.target.parentElement.getAttribute("placed")) {
                    ...
                    if (
                        ev.target.parentElement.parentElement.getAttribute(
                            "placed"
                        )
                    ) {
                        // rectange and oval
            ...
            if (selectedItem.id != mergingItem.id) {
                ...
                let selectedTable = tableforUi(selectedItem.id);
                let mergingTable = tableforUi(mergingItem.id);
    ...
    if (selectedItem) {
        if (selectedItem.getAttribute("selected") == "true") {
            let contextMenuItem = document.getElementById(
                "m" + selectedItem.id
            );
            if (contextMenuItem) {
